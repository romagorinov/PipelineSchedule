...
namespace Algorithms
{
    public class SectionWithPumpsMathModel : ISection
    {
        ...
        private IntervalsParameters _currentIntervalsParameters;
        ...
        public List<List<int>> ControlAvaliableIntervals => _currentIntervalsParameters.uniformityIntervals;
        ...
        public SectionWithPumpsMathModel(List<Tuple<double, double[][]>> regimes, double[] pumpSigns, List<Tuple<double, double[], double>> maxFlows, double[] weights = null)
        {
            if (pumpSigns == null)
                throw new Exception();
            if (pumpSigns.Count() == 0)
                throw new Exception();
            if (pumpSigns.Any(x => x != 1 && x != -1))
                throw new Exception();
            if (regimes == null)
                throw new Exception();
            if (regimes.Count() == 0)
                throw new Exception();
            if (regimes.Any(x => x == null))
                throw new Exception();
            if (regimes.Any(x => x.Item1 < 0))
                throw new Exception();
            if (regimes.Any(x => x.Item2 == null))
                throw new Exception();
            if (regimes.Any(x => x.Item2.Count() != pumpSigns.Count()))
                throw new Exception();
            if (regimes.Any(x => x.Item2.Any(y => y.Count() != 2)))
                throw new Exception();
            if (regimes.Any(x => x.Item2.Any(y => y[0] < 0 || y[1] < 0)))
                throw new Exception();
            if (regimes.Any(x => x.Item2.Any(y => y[0] > y[1])))
                throw new Exception();
            if (maxFlows == null)
                throw new Exception();
            if (maxFlows.Count() == 0)
                throw new Exception();
            if (maxFlows.Any(x => x == null))
                throw new Exception();
            if (maxFlows.Any(x => x.Item1 < 0 || x.Item3 < 0))
                throw new Exception();
            if (maxFlows.Any(x => x.Item2 == null))
                throw new Exception();
            if (maxFlows.Any(x => x.Item2.Count() != pumpSigns.Count()))
                throw new Exception();
            if (maxFlows.Any(x => x.Item2.Any(y => y < 0)))
                throw new Exception();
            if (weights != null && weights.Count() != pumpSigns.Count())
                throw new Exception();
        ...
        private class IntervalsParameters
        {
        ...
        private List<Tuple<List<double[]>, List<int>>> DecomposeVolumes(TargetVolumes volumes)
        {
            ...
            foreach(var tuple in volumes.targetVolumes)
            {
                ...
                {
                    int idx = indexes[i];
                    if (_repairs[idx] != _notRepair || volumes.GetFix(idx) != null)
                    {
        ...
        private List<double[]> GetDiscreteSchedule(double inVolume, double[] pumpsVolume, List<int> indexes)
        {
            ...
            for (int i = 0; i < sameParamsIntervals.Count(); i++)
            {
                ...
                else
                {
                    ...
                    RegimeMathModel targetRegime = new RegimeMathModel(sumOnCurrentInterval[0] / currentPeriod, new double[_pumpsCount], new double[_pumpsCount], _pumpsSigns);
        ...
        private double[] AddOutputElement(double[] val)
        {
            var list = val.ToList();
        ...
        public void CalcDefaultIntervalsParameters(TargetVolumes volumes)
        {
            ...
            foreach (var tuple in volumes.targetVolumes)
            {
                ...
                foreach (var idx in indexes)
                {
                    ...
                    if (fixVal != null)
                        avaliableRegimesOnIntervals[idx] = new HashSet<RegimeMathModel>(new List<RegimeMathModel> { _regimes.First(x => x.CanProvide(fixVal)) });
                }
            }
            ...
            _currentIntervalsParameters = new IntervalsParameters()
            {
                ...
                uniformityIntervals = volumes.targetVolumes.Select(kv => kv.Item2.Where(x => _repairs[x] == _notRepair && volumes.GetFix(x) == null).ToList()).ToList()
            };
        ...
        {
            CheckSchedule(schedule, Dimension);
